...
try:
    ...
    MLFLOW_TRACKING_URI = os.getenv("MLFLOW_TRACKING_URI", f"file:///{os.path.abspath('./mlruns').replace(chr(92), '/')}")
    mlflow.set_tracking_uri(MLFLOW_TRACKING_URI)
    MLFLOW_AVAILABLE = True
    logger.info("Connected to MLflow tracking engine successfully.")
except Exception as e:
    logger.warning(f"MLflow client library not connected, falling back to local file-system experiment store: {e}")
...
class MLflowService:
    @staticmethod
    def log_run(run_id: str, name: str, algorithm: str, parameters: dict, metrics: dict, status: str = "SUCCESS") -> dict:
        """
